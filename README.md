...
- [jenkins](https://github.com/fivetran/fivetran_connector_sdk/tree/main/connectors/jenkins) - This example shows how to sync data from Jenkins using Connector SDK. It connects to the Jenkins REST API using basic authentication with an API token, retrieves job and build information, and syncs it into your Fivetran destination. The connector supports incremental synchronization based on build timestamps and processes artifacts only for new builds since the last sync. The connector handles retries, rate limiting, and checkpointing for efficient and reliable data sync.
