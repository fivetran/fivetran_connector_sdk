...
- [ibm_db2](https://github.com/fivetran/fivetran_connector_sdk/tree/main/connectors/ibm/ibm_db2) - This example shows how to connect and sync data from IBM Db2 using Connector SDK. It uses the `ibm_db` library to connect to the database and fetch data.
...
- [backoff](https://github.com/fivetran/fivetran_connector_sdk/tree/main/examples/common_patterns_for_connectors/backoff) - This example demonstrates retry and backoff strategies (`fixed`, `linear`, `exponential`, `exponential_with_cap`, `exponential_with_jitter`, `retry_after`) for handling rate limits and transient API failures while syncing paginated data.
...
- [database_limit_offset](https://github.com/fivetran/fivetran_connector_sdk/tree/main/examples/common_patterns_for_connectors/pagination/database_limit_offset) - This is a simple example of how to work with LIMIT/OFFSET pagination for a database source.
...
- [truncate_operation](https://github.com/fivetran/fivetran_connector_sdk/tree/main/examples/common_patterns_for_connectors/truncate_operation) - This example demonstrates the truncate operation combined with `upsert()`, `update()`, and `delete()`. It shows how `truncate()` soft-deletes all existing rows in a table, how rows upserted after `truncate()₹ in the same sync are unaffected, and how upserting a previously truncated row revives it.
