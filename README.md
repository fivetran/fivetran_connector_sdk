...
- [scylla_db](https://github.com/fivetran/fivetran_connector_sdk/tree/main/connectors/scylla_db) - This connector extracts data from a **Cassandra** or **ScyllaDB** cluster and streams it into your Fivetran destination using the **Fivetran Connector SDK**. It performs **incremental extraction** based on a timestamp column (`updated_at`) and supports **checkpointing** for reliable, resumable syncs. The connector discovers all tables in a keyspace automatically and supports upserts for every record.
