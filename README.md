...
- [history_mode_mimicry](https://github.com/fivetran/fivetran_connector_sdk/tree/main/examples/common_patterns_for_connectors/history_mode_mimicry) - This example demonstrates how to mimic Fivetran History Mode using a composite primary key. The Connector SDK does not natively support History Mode, but by including a timestamp column (e.g. `_LastUpdatedInstant`) as part of the composite primary key, each update to a record produces a new row rather than overwriting the existing one. Uses a SQL Server source via `python-tds`.
