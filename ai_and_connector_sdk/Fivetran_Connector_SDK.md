...
- Implementation of required operations:
  * Upsert: Use `yield op.upsert(table, data)` for creating/updating records
  * Update: Use `yield op.update(table, modified)` for updating existing records
  * Delete: Use `yield op.delete(table, keys)` for marking records as deleted
  * Checkpoint: Use `yield op.checkpoint(state)` for incremental syncs
