...
class FDADrugConnector:
    ...
    def __init__(self, configuration: Dict[str, str]):
        ...
        self.rate_limit_delay = float(configuration.get("rate_limit_delay", "0.25"))  # 250ms between requests
        self.flatten_nested = configuration.get("flatten_nested", "true").lower() == "true"
        self.create_child_tables = configuration.get("create_child_tables", "false").lower() == "true"
        ...
        log.info(f"Initialized FDA Drug Connector with {self.requests_per_checkpoint} requests per checkpoint")
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        ...
        headers = {
            "User-Agent": "Fivetran-FDA-Connector/1.0",
    ...
    def _extract_child_tables(self, data: Dict[str, Any], parent_key: str) -> Tuple[Dict[str, Any], Dict[str, List[Dict[str, Any]]]]:
    ...
    def sync_endpoint(self, endpoint: str, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while api_call_count < self.max_api_calls_per_endpoint:
            ...
            if not response or "results" not in response:
                log.warning(f"No more data available for {endpoint} after {api_call_count} API calls")
                break
            ...
            for record in results:
                ...
                if self.create_child_tables:
                    ...
                    processed_record = self._flatten_dict(record) if self.flatten_nested else record
            ...
            if checkpoint_counter >= self.requests_per_checkpoint:
                ...
                checkpoint_counter = 0
                log.info(f"Checkpoint complete for {endpoint}, total records: {total_records}, API calls: {api_call_count}")
        ...
        if api_call_count >= self.max_api_calls_per_endpoint:
            log.info(f"Reached maximum API calls ({self.max_api_calls_per_endpoint}) for {endpoint}")
        ...
        log.info(f"Completed sync for {endpoint}: {total_records} records processed, {api_call_count} API calls used")
...
def update(configuration: dict, state: dict):
    ...
    log.info(f"Processing {len(endpoints)} endpoints with max {max_calls_per_endpoint} API calls each")
    log.info(f"Total estimated API calls: {len(endpoints) * max_calls_per_endpoint}")
    
    try:
        for endpoint in endpoints:
            log.info(f"Starting sync for endpoint: {endpoint} (max {max_calls_per_endpoint} API calls)")
        ...
        if connector.api_key:
            ...
            if total_api_calls > 500:
                log.warning("Approaching daily API limit - consider getting an API key for higher limits")
...
if __name__ == "__main__":
    ...
    with open("/Users/elijah.davis/Documents/code/sdk/customer/ai_customer/FDA_drug_claude_no_yield/configuration.json", 'r') as f:
