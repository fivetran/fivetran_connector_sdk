#!/usr/bin/env bash
# Pre-submission gate for the FDA FAERS Pharmacovigilance connector.
...
#
# Important caveats baked in here (PR #570 + #571 lessons):
#   - flake8 runs on `connector.py tests/` (the WHOLE connector dir),
#     because CI runs flake8 on every changed `.py` file.
#   - black is upgraded to latest before running so the local check
#     matches CI's unpinned `pip install black`.
set -euo pipefail
...
echo "=== 1/7 flake8 (connector.py + tests/) ==="
if [ -d tests ]; then
    python3 -m flake8 connector.py tests/
else
...
echo "=== 4/7 PII scan (emails in source, embedded tokens) ==="
