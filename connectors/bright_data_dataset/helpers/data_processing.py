"""Data processing utilities for flattening and normalizing Bright Data dataset results."""
...
def process_dataset_record(
    record: Any, record_index: int, dataset_id: str
) -> Dict[str, Any]:
    """
    Process a single dataset record by flattening nested dictionaries.

    Args:
        record: Dataset record dictionary returned by the API
        record_index: Index of this record in the result set
        dataset_id: The dataset ID that was queried
    ...
    if not isinstance(record, dict):
        base_fields["raw_data"] = str(record)
        return base_fields
