"""Countly connector for syncing analytics data including apps, groups, users, and performance metrics.
This connector demonstrates how to fetch data from Countly Analytics API and upsert it into destination using memory-efficient streaming patterns.
...
# For making HTTP requests to Countly Analytics API
import requests
...
__API_ENDPOINT = "https://api.count.ly"  # Countly API base URL
...
def execute_api_request(endpoint, api_key, params=None, configuration=None):
    ...
    # Countly uses API key as query parameter
    if params is None:
        params = {}
...
def __map_app_data(record, additional_context=None):
    """
    Transform API response record to destination table schema format for apps.
    ...
    return {
        ...
        "created_at": record.get(
            "created_at", record.get("created", datetime.now(timezone.utc).isoformat())
        ),
        "updated_at": record.get(
            "updated_at", record.get("updated", datetime.now(timezone.utc).isoformat())
        ),
...
def __map_group_data(record, additional_context=None):
    """
    Transform API response record to destination table schema format for groups.
    This function maps raw group API fields to normalized database column names.
    ...
    return {
        ...
        "user_count": record.get("user_count", record.get("users", 0)),
        "created_at": record.get(
            "created_at", record.get("created", datetime.now(timezone.utc).isoformat())
        ),
        "updated_at": record.get(
...
def __map_user_data(record, additional_context=None):
    """
    Transform API response record to destination table schema format for users.
    This function maps raw user API fields to normalized database column names.
    ...
    return {
        ...
        "byear": record.get("byear", 0),
        "custom_data": json.dumps(record.get("custom", {})),
        "first_seen": record.get("fs", ""),
        "last_seen": record.get("ls", ""),
        "session_count": record.get("sc", 0),
        "total_session_duration": record.get("tsd", 0),
        "processed_at": datetime.now(timezone.utc).isoformat(),
    }


def get_apps_data(api_key, params, last_sync_time=None, configuration=None):
    """
    Fetch apps data using memory-efficient streaming approach with pagination.
    ...
    Yields:
        dict: Individual app records mapped to destination schema.
...
def get_groups_data(api_key, params, last_sync_time=None, configuration=None):
    """
...
def get_users_data(api_key, params, last_sync_time=None, configuration=None):
    """
    ...
    endpoint = "/o/users/all"
    max_records = __get_config_int(configuration, "max_records_per_page", 100)

    request_params = {
        "period": f"{time_range['start']},{time_range['end']}",
        "limit": max_records,
        "skip": 0,
    }
    request_params.update(params)
...
def schema(configuration: dict):
    ...
    return [
        ...
        {"table": "users", "primary_key": ["user_id"]},
    ]
    ...
    try:
        ...
        if __get_config_bool(configuration, "enable_apps_sync", True):
            ...
            for record in get_apps_data(api_key, {}, last_sync_time, configuration):
                ...
                op.upsert(table="apps", data=record)
                apps_count += 1

                # Checkpoint every page/batch to save progress incrementally
                if apps_count % max_records_per_page == 0:
                    checkpoint_state = {
                        "last_sync_time": record.get(
                            "updated_at", datetime.now(timezone.utc).isoformat()
                        ),
                        "last_processed_apps_page": page,
                    }
        ...
        if __get_config_bool(configuration, "enable_groups_sync", True):
            ...
            for record in get_groups_data(api_key, {}, last_sync_time, configuration):
                ...
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="groups", data=record)
                groups_count += 1
