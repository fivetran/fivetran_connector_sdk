...
This connector demonstrates how to fetch Discord server data including guilds, channels, messages, and users using the Discord API.
...
# USERS CAN MODIFY: Adjust these values based on your Discord API usage patterns and data volume
    ...
    for attempt in range(__MAX_RETRIES):
        try:
            ...
            if response.status_code == 200:
                ...
                log.warning(f"Rate limited. Waiting {retry_after} seconds before retry (attempt {attempt + 1}/{__MAX_RETRIES})")
                time.sleep(retry_after)
                continue
            elif response.status_code in [500, 502, 503, 504]:  # Server errors
                if attempt < __MAX_RETRIES - 1:
                    delay = __BASE_DELAY * (2 ** attempt)  # Exponential backoff
                    log.warning(f"Server error {response.status_code}, retrying in {delay} seconds (attempt {attempt + 1}/{__MAX_RETRIES})")
                    time.sleep(delay)
                    continue
                else:
                    log.severe(f"Server error {response.status_code} after {__MAX_RETRIES} attempts: {response.text}")
                    raise RuntimeError(f"Discord API returned {response.status_code} after {__MAX_RETRIES} attempts")
            else:
            ...
            if attempt < __MAX_RETRIES - 1:
                ...
                log.warning(f"Request failed: {str(e)}, retrying in {delay} seconds (attempt {attempt + 1}/{__MAX_RETRIES})")
                time.sleep(delay)
                continue
            else:
                log.severe(f"Request failed after {__MAX_RETRIES} attempts: {str(e)}")
                raise RuntimeError(f"Discord API request failed after {__MAX_RETRIES} attempts: {str(e)}")
...
def filter_guilds(guilds: List[dict], configuration: dict) -> List[dict]:
    ...
    if not sync_all_guilds and not specific_guild_ids:
        log.warning("sync_all_guilds is false but no specific guild_ids provided. Processing all guilds.")
        return guilds
    ...
    log.info(f"Filtered {len(guilds)} guilds down to {len(filtered_guilds)} guilds for processing")
...
def fetch_channel_messages(channel_id: str, headers: dict, after: Optional[str] = None, limit: int = 100) -> List[dict]:
    """
...
def process_guild_data(guild_data: dict) -> dict:
    ...
    return {
        ...
        "default_message_notifications": guild_data.get("default_message_notifications"),
        ...
        "welcome_screen": json.dumps(guild_data.get("welcome_screen")) if guild_data.get("welcome_screen") else None,
        "nsfw_level": guild_data.get("nsfw_level"),
        "stickers": json.dumps(guild_data.get("stickers")) if guild_data.get("stickers") else None,
...
def process_channel_data(channel_data: dict, guild_id: str) -> dict:
    ...
    return {
        ...
        "permission_overwrites": json.dumps(channel_data.get("permission_overwrites")) if channel_data.get("permission_overwrites") else None,
        ...
        "default_auto_archive_duration": channel_data.get("default_auto_archive_duration"),
        "flags": channel_data.get("flags"),
        "available_tags": json.dumps(channel_data.get("available_tags")) if channel_data.get("available_tags") else None,
        "default_reaction_emoji": json.dumps(channel_data.get("default_reaction_emoji")) if channel_data.get("default_reaction_emoji") else None,
        "default_thread_rate_limit_per_user": channel_data.get("default_thread_rate_limit_per_user"),
...
def process_message_data(message_data: dict, channel_id: str) -> dict:
    ...
    return {
        ...
        "activity": json.dumps(message_data.get("activity")) if message_data.get("activity") else None,
        "application": json.dumps(message_data.get("application")) if message_data.get("application") else None,
        "application_id": message_data.get("application_id"),
        "message_reference": json.dumps(message_data.get("message_reference")) if message_data.get("message_reference") else None,
        "flags": message_data.get("flags"),
        "referenced_message": json.dumps(message_data.get("referenced_message")) if message_data.get("referenced_message") else None,
        "interaction": json.dumps(message_data.get("interaction")) if message_data.get("interaction") else None,
        "thread": json.dumps(message_data.get("thread")) if message_data.get("thread") else None,
        ...
        "role_subscription_data": json.dumps(message_data.get("role_subscription_data")) if message_data.get("role_subscription_data") else None,
        "synced_at": datetime.now(timezone.utc).isoformat()
    }
...
def process_single_guild(guild_data: dict, headers: dict, configuration: dict, state: dict) -> dict:
    """
    ...
    try:
        ...
        # This ensures that if a guild already exists, it will be updated with the latest information.
        ...
        for channel_data in channels_data:
            ...
            # The 'upsert' operation is used to insert or update data in the destination table.
            # This ensures that if a channel already exists, it will be updated with the latest information.
        ...
        for member_data in members_data:
            ...
            # The 'upsert' operation is used to insert or update data in the destination table.
            # This ensures that if a member already exists, it will be updated with the latest information.
        ...
        if sync_messages:
            ...
            text_channels = [ch for ch in channels_data if ch.get("type") in [0, 5]]  # Text and news channels
            ...
            for channel in text_channels:
                ...
                # This ensures that the connector does not load all data into memory at once
                while messages_fetched < message_limit:
                    try:
                        # Fetch messages for this channel using cursor-based pagination
                        # The 'after' parameter ensures we only get messages newer than the last processed message
                        ...
                        for message_data in messages_data:
                            ...
                            # The 'upsert' operation is used to insert or update data in the destination table.
                            # This ensures that if a message already exists, it will be updated with the latest information.
                        ...
                        log.warning(f"Error fetching messages for channel {channel_name} in guild {guild_name}: {str(e)}")
                        break
                
                # Update state with last message ID for this channel
                last_message_sync[channel_id] = last_message_id
                log.info(f"Processed {messages_fetched} messages for channel: {channel_name} in guild: {guild_name}")
    ...
    try:
        ...
        for guild_data in guilds_to_process:
            ...
            try:
                ...
                log.info(f"Completed processing guild: {guild_name} - {guild_state.get('processed_count', 0)} records")
        ...
        log.info(f"Discord Connector: Multi-guild sync completed successfully. Processed {total_processed_count} total records across {len(guilds_to_process)} guilds")
