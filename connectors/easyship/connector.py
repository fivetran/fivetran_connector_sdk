"""Easyship API connector for syncing boxes, manifests, and pickups data.
This connector demonstrates how to fetch data from Easyship API and upsert it into destination using memory-efficient streaming patterns.
...
# For making HTTP requests to Easyship API
import requests
...
__API_ENDPOINT = "https://public-api.easyship.com/2024-09"
...
def __map_box_data(record):
    """
    Transform API response record to boxes table schema format.
    This function maps raw API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for box data.
    ...
    return {
        ...
        "is_custom": record.get("is_custom", False),
...
def __map_manifest_data(record):
    """
    Transform API response record to manifests table schema format.
    This function maps raw API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for manifest data.
    ...
    return {
        "id": record.get("id", ""),
        "manifest_number": record.get("manifest_number", ""),
        "status": record.get("status", ""),
        "courier_id": record.get("courier_id"),
        "courier_name": record.get("courier_name", ""),
        "shipment_count": record.get("shipment_count", 0),
...
def __map_pickup_data(record):
    """
    Transform API response record to pickups table schema format.
    This function maps raw API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for pickup data.
    ...
    return {
        "id": record.get("id", ""),
        "pickup_number": record.get("pickup_number", ""),
        "status": record.get("status", ""),
        "courier_id": record.get("courier_id"),
        "courier_name": record.get("courier_name", ""),
        "pickup_date": record.get("pickup_date", ""),
        "pickup_time_from": record.get("pickup_time_from", ""),
        "pickup_time_to": record.get("pickup_time_to", ""),
...
def get_boxes_data(api_key, last_sync_time=None, configuration=None):
    """
    Fetch boxes data using memory-efficient streaming approach.
    ...
    Yields:
        dict: Individual box records mapped to destination schema.
    ...
    # Add time-based filters if available in API
    ...
    boxes_data = response.get("boxes", response.get("data", []))

    for record in boxes_data:
        yield __map_box_data(record)


def get_manifests_data(api_key, last_sync_time=None, configuration=None):
    """
    Fetch manifests data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual manifest records mapped to destination schema.
    ...
    while True:
        ...
        # Handle different response formats
        manifests_data = response.get("manifests", response.get("data", []))

        if not manifests_data:
            break

        for record in manifests_data:
            yield __map_manifest_data(record)

        # Check pagination
        if len(manifests_data) < max_records:
            break

        params["offset"] += max_records


def get_pickups_data(api_key, last_sync_time=None, configuration=None):
    """
    Fetch pickups data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual pickup records mapped to destination schema.
    ...
    while True:
        ...
        # Handle different response formats
        pickups_data = response.get("pickups", response.get("data", []))

        if not pickups_data:
            break

        for record in pickups_data:
            yield __map_pickup_data(record)

        # Check pagination
        if len(pickups_data) < max_records:
            break
...
def schema(configuration: dict):
    ...
    """
    return [
        {"table": "boxes", "primary_key": ["id"]},
        {"table": "manifests", "primary_key": ["id"]},
        {"table": "pickups", "primary_key": ["id"]},
    ]


def update(configuration: dict, state: dict):
    """
    Main synchronization function that fetches and processes data from the Easyship API.
    ...
    """
    log.info("Starting Easyship API connector sync")
    ...
    try:
        # Fetch boxes data
        log.info("Fetching boxes data...")
        boxes_count = 0

        for record in get_boxes_data(api_key, last_sync_time, configuration):
            # The 'upsert' operation is used to insert or update data in the destination table.
            # The op.upsert method is called with two arguments:
            # - The first argument is the name of the table to upsert the data into.
            # - The second argument is a dictionary containing the data to be upserted,
            op.upsert(table="boxes", data=record)
            boxes_count += 1

            # Checkpoint every batch to save progress incrementally
            if boxes_count % max_records_per_page == 0:
                checkpoint_state = {
                    "last_sync_time": record.get(
                        "updated_at", datetime.now(timezone.utc).isoformat()
                    ),
                    "boxes_processed": boxes_count,
                }
        ...
        log.info(f"Fetched {boxes_count} boxes")

        # Fetch manifests data
        log.info("Fetching manifests data...")
        manifests_count = 0

        for record in get_manifests_data(api_key, last_sync_time, configuration):
            # The 'upsert' operation is used to insert or update data in the destination table.
            # The op.upsert method is called with two arguments:
            # - The first argument is the name of the table to upsert the data into.
            # - The second argument is a dictionary containing the data to be upserted,
            op.upsert(table="manifests", data=record)
            manifests_count += 1

            # Checkpoint every batch to save progress incrementally
            if manifests_count % max_records_per_page == 0:
                checkpoint_state = {
                    "last_sync_time": record.get(
                        "updated_at", datetime.now(timezone.utc).isoformat()
                    ),
                    "manifests_processed": manifests_count,
                }
                # Save the progress by checkpointing the state. This is important for ensuring that the sync process can resume
                # from the correct position in case of next sync or interruptions.
                # Learn more about how and where to checkpoint by reading our best practices documentation
        ...
        log.info(f"Fetched {manifests_count} manifests")

        # Fetch pickups data
        log.info("Fetching pickups data...")
        pickups_count = 0

        for record in get_pickups_data(api_key, last_sync_time, configuration):
            # The 'upsert' operation is used to insert or update data in the destination table.
            # The op.upsert method is called with two arguments:
            # - The first argument is the name of the table to upsert the data into.
            # - The second argument is a dictionary containing the data to be upserted,
            op.upsert(table="pickups", data=record)
            pickups_count += 1

            # Checkpoint every batch to save progress incrementally
            if pickups_count % max_records_per_page == 0:
                checkpoint_state = {
                    "last_sync_time": record.get(
                        "updated_at", datetime.now(timezone.utc).isoformat()
                    ),
                    "pickups_processed": pickups_count,
                }
                # Save the progress by checkpointing the state. This is important for ensuring that the sync process can resume
                # from the correct position in case of next sync or interruptions.
                # Learn more about how and where to checkpoint by reading our best practices documentation
        ...
        log.info(f"Fetched {pickups_count} pickups")

        # Final checkpoint with completion status
        final_state = {
            "last_sync_time": datetime.now(timezone.utc).isoformat(),
            "boxes_total": boxes_count,
            "manifests_total": manifests_count,
            "pickups_total": pickups_count,
        }
        # Save the progress by checkpointing the state. This is important for ensuring that the sync process can resume
        # from the correct position in case of next sync or interruptions.
        # Learn more about how and where to checkpoint by reading our best practices documentation
        ...
        total_records = boxes_count + manifests_count + pickups_count
