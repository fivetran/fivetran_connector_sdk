...
2. Authentication flow (handled automatically by the connector - see `get_oauth_token()` function):
...
## Pagination
The connector implements offset-based pagination to handle large order datasets (see `update()` function, and `fetch_orders_page()` function):
...
### Data transformation (see `flatten_dict()` function)
...
### Data processing flow (see `process_single_order()` and related functions)
...
### State management (see `update()` function)
...
### Individual order errors (see `update()` function)
