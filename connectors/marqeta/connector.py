"""Marqeta API connector for syncing users, businesses, and transactions data.
This connector demonstrates how to fetch data from Marqeta Core API and upsert it into destination using memory-efficient streaming patterns.
...
# For making HTTP requests to Marqeta API
import requests
...
__API_ENDPOINT = "https://api.marqeta.com/v3"
...
def execute_api_request(endpoint, auth_credentials, params=None, configuration=None):
    """
    ...
    Args:
        ...
        auth_credentials: Dictionary containing username and password for Basic auth.
...
def __map_user_data(record):
    ...
    This function maps raw Marqeta user API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for user data.
    ...
    return {
        ...
        "account_holder_group_token": record.get("account_holder_group_token", ""),
        "status": record.get("status", ""),
        "uses_parent_account": record.get("uses_parent_account", False),
        "parent_token": record.get("parent_token", ""),
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }


def __map_business_data(record):
    """
    Transform API response record to businesses table schema format.
    This function maps raw Marqeta business API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for business data.

    Returns:
        dict: Transformed business record ready for database insertion.
    """
    return {
        "token": record.get("token", ""),
        "active": record.get("active", True),
        "business_name_legal": record.get("business_name_legal", ""),
        "business_name_dba": record.get("business_name_dba", ""),
...
def __map_transaction_data(record):
    """
    Transform API response record to transactions table schema format.
    This function maps raw Marqeta transaction API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary for transaction data.

    Returns:
        dict: Transformed transaction record ready for database insertion.
    """
    return {
        "token": record.get("token", ""),
        ...
        "business_token": record.get("business_token", ""),
        "card_token": record.get("card_token", ""),
        "amount": record.get("amount", 0.0),
        "currency_code": record.get("currency_code", ""),
        "request_amount": record.get("request_amount", 0.0),
        "network": record.get("network", ""),
        "subnetwork": record.get("subnetwork", ""),
        "acquirer_fee_amount": record.get("acquirer_fee_amount", 0.0),
        "created_time": record.get("created_time", ""),
        "user_transaction_time": record.get("user_transaction_time", ""),
        "settlement_date": record.get("settlement_date", ""),
        "merchant": json.dumps(record.get("merchant", {})) if record.get("merchant") else None,
        "card_acceptor": (
            json.dumps(record.get("card_acceptor", {})) if record.get("card_acceptor") else None
        ),
        "acquirer": json.dumps(record.get("acquirer", {})) if record.get("acquirer") else None,
        "issuer": json.dumps(record.get("issuer", {})) if record.get("issuer") else None,
        "response": json.dumps(record.get("response", {})) if record.get("response") else None,
        "metadata": json.dumps(record.get("metadata", {})) if record.get("metadata") else None,
        "clearing_record_sequence_number": record.get("clearing_record_sequence_number", ""),
        "network_reference_id": record.get("network_reference_id", ""),
        "approval_code": record.get("approval_code", ""),
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }


def get_users(auth_credentials, params, last_sync_time=None, configuration=None):
    """
    ...
    Args:
        auth_credentials: Dictionary containing authentication credentials.
    ...
    if last_sync_time:
        request_params["start_date"] = last_sync_time

    start_index = 0
    while True:
        request_params["start_index"] = start_index
        response = execute_api_request(endpoint, auth_credentials, request_params, configuration)
...
def get_businesses(auth_credentials, params, last_sync_time=None, configuration=None):
    """
    Fetch businesses data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual business records mapped to destination schema.
    ...
    while True:
        ...
        # Yield individual records instead of accumulating
        for record in data:
            yield __map_business_data(record)
...
def get_transactions(auth_credentials, params, last_sync_time=None, configuration=None):
    """
    Fetch transactions data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual transaction records mapped to destination schema.
    ...
    while True:
        ...
        # Yield individual records instead of accumulating
        for record in data:
            yield __map_transaction_data(record)
...
def schema(configuration: dict):
    ...
    """
    return [
        {"table": "users", "primary_key": ["token"]},
        {"table": "businesses", "primary_key": ["token"]},
        {"table": "transactions", "primary_key": ["token"]},
    ]


def update(configuration: dict, state: dict):
    """
    Main synchronization function that fetches and processes data from the Marqeta API.
    ...
    """
    log.info("Starting Marqeta API connector sync")
    ...
    max_records_per_page = __get_config_int(configuration, "max_records_per_page", 100, 1, 500)
    ...
    try:
        ...
        user_count = 0
        for record in get_users(auth_credentials, {}, last_sync_time, configuration):
        ...
        log.info(f"Completed users sync. Processed {user_count} users.")

        # Fetch businesses data using generator with incremental checkpointing
        log.info("Fetching businesses data...")
        business_count = 0
        for record in get_businesses(auth_credentials, {}, last_sync_time, configuration):
            # The 'upsert' operation is used to insert or update data in the destination table.
            # The op.upsert method is called with two arguments:
            # - The first argument is the name of the table to upsert the data into.
            # - The second argument is a dictionary containing the data to be upserted,
            op.upsert(table="businesses", data=record)
            business_count += 1

            # Checkpoint every page/batch to save progress incrementally
            if business_count % max_records_per_page == 0:
                checkpoint_state = {
                    "last_sync_time": record.get(
                        "last_modified_time", datetime.now(timezone.utc).isoformat()
                    ),
                    "users_processed": user_count,
                    "businesses_processed": business_count,
                }
                # Save the progress by checkpointing the state. This is important for ensuring that the sync process can resume
                # from the correct position in case of next sync or interruptions.
                # Learn more about how and where to checkpoint by reading our best practices documentation
        ...
        log.info(f"Completed businesses sync. Processed {business_count} businesses.")

        # Fetch transactions data using generator with incremental checkpointing
        log.info("Fetching transactions data...")
        transaction_count = 0
        for record in get_transactions(auth_credentials, {}, last_sync_time, configuration):
            # The 'upsert' operation is used to insert or update data in the destination table.
            # The op.upsert method is called with two arguments:
            # - The first argument is the name of the table to upsert the data into.
            # - The second argument is a dictionary containing the data to be upserted,
            op.upsert(table="transactions", data=record)
            transaction_count += 1

            # Checkpoint every page/batch to save progress incrementally
            if transaction_count % max_records_per_page == 0:
                checkpoint_state = {
                    "last_sync_time": record.get(
        ...
        log.info(f"Completed transactions sync. Processed {transaction_count} transactions.")
        ...
        log.info(
            f"Sync completed successfully. Total: {user_count} users, {business_count} businesses, {transaction_count} transactions."
        )
...
# Creating an instance of the connector
