"""Meltwater API connector for syncing hooks, searches, and tags data.
This connector demonstrates how to fetch data from Meltwater API and upsert it into destination using memory-efficient streaming patterns.
...
# For making HTTP requests to Meltwater API
import requests
...
__API_ENDPOINT = "https://api.meltwater.com/v2"
...
def __map_hook_data(record, additional_context=None):
    """
    Transform API response record to hooks table schema format.
    This function maps raw API fields to normalized database column names and types.

    Args:
        record: Raw API response record dictionary.
        additional_context: Additional context information for mapping.
    ...
    return {
        ...
        "event_types": json.dumps(record.get("event_types", [])),
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }


def __map_search_data(record, additional_context=None):
    """
    Transform API response record to searches table schema format.
    This function maps raw API fields to normalized database column names and types.
    ...
    return {
        ...
        "source_types": json.dumps(record.get("source_types", [])),
        "created_at": record.get("created_at", datetime.now(timezone.utc).isoformat()),
        "updated_at": record.get("updated_at", datetime.now(timezone.utc).isoformat()),
...
def __map_tag_data(record, additional_context=None):
    """
    Transform API response record to tags table schema format.
    This function maps raw API fields to normalized database column names and types.
...
def get_hooks_data(api_key, params, last_sync_time=None, configuration=None):
    """
    Fetch hooks data using memory-efficient streaming approach with pagination.
    ...
    Yields:
        dict: Individual hook records mapped to destination schema.
...
def get_searches_data(api_key, params, last_sync_time=None, configuration=None):
    """
    Fetch searches data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual search records mapped to destination schema.
    ...
    while True:
        ...
        # Yield individual records instead of accumulating
        for record in data:
            yield __map_search_data(record)
...
def get_tags_data(api_key, params, last_sync_time=None, configuration=None):
    """
    Fetch tags data using memory-efficient streaming approach with pagination.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual tag records mapped to destination schema.
...
def schema(configuration: dict):
    ...
    """
    return [
        {"table": "hooks", "primary_key": ["id"]},
        {"table": "searches", "primary_key": ["id"]},
...
def update(configuration: dict, state: dict):
    """
    Main synchronization function that fetches and processes data from the Meltwater API.
    ...
    """
    log.info("Starting Meltwater API connector sync")
    ...
    # enable_incremental = __get_config_bool(configuration, "enable_incremental_sync", True)  # Currently unused
    enable_hooks = __get_config_bool(configuration, "enable_hooks", True)
    enable_searches = __get_config_bool(configuration, "enable_searches", True)
    enable_tags = __get_config_bool(configuration, "enable_tags", True)

    # Get state for incremental sync
    last_sync_time = state.get("last_sync_time")

    try:
        # Fetch hooks data using generator with incremental checkpointing
        if enable_hooks:
            log.info("Fetching hooks data...")
            record_count = 0
            page = 1

            for record in get_hooks_data(api_key, {}, last_sync_time, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="hooks", data=record)
                record_count += 1
            ...
            log.info(f"Processed {record_count} hooks records")

        # Fetch searches data if enabled
        if enable_searches:
            log.info("Fetching searches data...")
            record_count = 0
            page = 1

            for record in get_searches_data(api_key, {}, last_sync_time, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="searches", data=record)
                record_count += 1
                ...
                if record_count % max_records_per_page == 0:
                    checkpoint_state = {
                        ...
                        "last_processed_page": page,
                        "searches_processed": record_count,
                    }
                    # Save the progress by checkpointing the state. This is important for ensuring that the sync process can resume
                    # from the correct position in case of next sync or interruptions.
                    # Learn more about how and where to checkpoint by reading our best practices documentation
            ...
            log.info(f"Processed {record_count} searches records")

        # Fetch tags data if enabled
        if enable_tags:
            log.info("Fetching tags data...")
            record_count = 0
            page = 1

            for record in get_tags_data(api_key, {}, last_sync_time, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="tags", data=record)
                record_count += 1
            ...
            log.info(f"Processed {record_count} tags records")
        ...
        log.info("Meltwater API sync completed successfully")
