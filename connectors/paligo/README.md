...
- Basic Authentication with automatic credential encoding (refer to the `execute_api_request` function)
- Page-based pagination with automatic page traversal (refer to the `get_users_data`, `get_taxonomies_data`, and `get_assignments_data` functions)
...
- Comprehensive error handling with exponential backoff retry logic (refer to the `__handle_request_error` and `__handle_rate_limit` functions)
