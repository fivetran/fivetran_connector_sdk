...
The PayScore connector extracts data from PayScore's income verification API and loads it into your configured destination warehouse. PayScore provides automated income verification services for financial institutions, enabling streamlined income verification processes through API integration. The connector supports incremental synchronization, handles API rate limiting, and provides comprehensive error handling for reliable data integration.
...
1. Log in to your [PayScore dashboard](https://app.payscore.com/users/sign_in).
...
4. Make a note of the API key to your `configuration.json` file.
...
PayScore API uses page-based pagination with the following strategy (refer to the `get_screening_groups`):
...
Memory efficiency:
- Generator-based processing: All data fetching functions use Python generators to yield individual records (refer to data mapping functions, such as `__map_screening_group_data`)
...
The connector implements comprehensive error handling with modular design (refer to the `execute_api_request` and helper functions):
...
The connector creates the `SCREENING_GROUP` table in your destination. Column types are automatically inferred by Fivetran.
