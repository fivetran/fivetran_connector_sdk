"""Rotamaster API connector for syncing employee, team, and role data.
This connector demonstrates how to fetch data from Rotamaster API and upsert it into destination using memory-efficient streaming patterns.
...
# For making HTTP requests to Rotamaster API
import requests
...
__API_ENDPOINT = "https://data-api.rotamasterweb.co.uk"
...
def __map_people_data(record):
    """
    Transform API response record to PEOPLE table schema format.
    ...
    return {
        ...
        "landline_number": record.get("landlineNumber", ""),
        "employment_start_date": record.get("employmentStartDate", ""),
        "employment_end_date": record.get("employmentEndDate", ""),
        "is_active": record.get("isActive", False),
        "is_archived": record.get("isArchived", False),
        "is_suspended": record.get("isSuspended", False),
        "is_deleted": record.get("isDeleted", False),
        "sync_timestamp": datetime.now(timezone.utc).isoformat(),
    }


def __map_teams_data(record):
    """
...
def __map_roles_data(record):
    """
    Transform API response record to ROLES table schema format.
    This function maps raw API fields to normalized database column names and types.
    ...
    return {
        "id": record.get("id", ""),
        "name": record.get("name", ""),
        "nominal_code": record.get("nominalCode", ""),
        "sync_timestamp": datetime.now(timezone.utc).isoformat(),
    }


def get_people_data(api_key, last_sync_time=None, configuration=None):
    """
    Fetch people data using memory-efficient streaming approach with pagination.
    ...
    Yields:
        dict: Individual people records mapped to destination schema.
    ...
    include_active = __get_config_bool(configuration, "include_active_people", True)
    include_archived = __get_config_bool(configuration, "include_archived_people", False)
    include_suspended = __get_config_bool(configuration, "include_suspended_people", False)
    include_deleted = __get_config_bool(configuration, "include_deleted_people", False)
    ...
    for record in data:
        yield __map_people_data(record)


def get_teams_data(api_key, configuration=None):
    """
    Fetch teams data using memory-efficient streaming approach.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    for record in data:
        yield __map_teams_data(record)


def get_roles_data(api_key, configuration=None):
    """
    Fetch roles data using memory-efficient streaming approach.
    This generator function prevents memory accumulation by yielding individual records.
    ...
    Yields:
        dict: Individual role records mapped to destination schema.
...
def schema(configuration: dict):
    ...
    return [
        ...
        {"table": "roles", "primary_key": ["id"]},
    ]


def update(configuration: dict, state: dict):
    """
    Main synchronization function that fetches and processes data from the Rotamaster API.
    ...
    """
    log.info("Starting Rotamaster API connector sync")
    ...
    enable_people_sync = __get_config_bool(configuration, "enable_people_sync", True)
    enable_teams_sync = __get_config_bool(configuration, "enable_teams_sync", True)
    enable_roles_sync = __get_config_bool(configuration, "enable_roles_sync", True)

    # Get state for incremental sync
    last_sync_time = state.get("last_sync_time")

    try:
        # Fetch people data if enabled
        if enable_people_sync:
            log.info("Fetching people data...")
            people_count = 0

            for record in get_people_data(api_key, last_sync_time, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="people", data=record)
                people_count += 1

                # Checkpoint every batch to save progress incrementally
                if people_count % max_records_per_page == 0:
                    checkpoint_state = {
            ...
            log.info(f"Completed people sync. Processed {people_count} records.")

        # Fetch teams data if enabled
        if enable_teams_sync:
            log.info("Fetching teams data...")
            teams_count = 0

            for record in get_teams_data(api_key, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="teams", data=record)
                teams_count += 1

            log.info(f"Completed teams sync. Processed {teams_count} records.")

        # Fetch roles data if enabled
        if enable_roles_sync:
            log.info("Fetching roles data...")
            roles_count = 0

            for record in get_roles_data(api_key, configuration):
                # The 'upsert' operation is used to insert or update data in the destination table.
                # The op.upsert method is called with two arguments:
                # - The first argument is the name of the table to upsert the data into.
                # - The second argument is a dictionary containing the data to be upserted,
                op.upsert(table="roles", data=record)
                roles_count += 1

            log.info(f"Completed roles sync. Processed {roles_count} records.")
        ...
        total_records = sum(
            [
                people_count if enable_people_sync else 0,
                teams_count if enable_teams_sync else 0,
                roles_count if enable_roles_sync else 0,
            ]
        )
        log.info(f"Sync completed successfully. Total processed: {total_records} records.")
