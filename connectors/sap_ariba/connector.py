...
def move_to_next_page(params: dict) -> None:
    ...
    """
    # Increase the page offset so we fetch the next page of results.
...
def convert_to_iso(date_str: str | None) -> str | None:
    ...
    if not date_str:
        return None
    try:
        # Example format: "16 May 2019 1:41:26 AM"
        parsed = datetime.strptime(date_str, "%d %b %Y %I:%M:%S %p")
        return parsed.strftime("%Y-%m-%dT%H:%M:%SZ")
    except ValueError:
        # If the format does not match, assume it is already in a usable format.
        return date_str
...
def filter_columns(record: dict, allowed_columns: dict) -> dict:
    ...
    for col_name, col_type in allowed_columns.items():
        if col_name == "rowId":
            # rowId is generated by the connector; skip reading from the source.
            continue
        if col_type == "UTC_DATETIME":
            # Convert any date-like field to ISO 8601 format.
...
def sync_orders(params: dict, headers: dict, state: dict, sync_start: str) -> None:
    ...
    # Start paging from the beginning each sync.
...
def handle_response(
    ...
    if response.status_code == 429:
        # Rate limit hit: back off and signal to retry.
        wait = delay * attempt
    ...
    if 400 <= response.status_code < 500:
        # Client errors are considered fatal for this request.
...
# the command line or IDE "run" button. This is useful for debugging while you write
# your code. Note this method is not called by Fivetran when executing your connector
# in production. Please test using the Fivetran debug command prior to finalizing
