...
2. Your connector creates a `FileUpload` object with the path for the file in your destination and a stream to read from (`FileUpload(path="file.pdf", stream=...)`).
3. Your connector calls `upsert()` with the file parameter to upload both the file and metadata (`op.upsert(table, data, file=...)`).
