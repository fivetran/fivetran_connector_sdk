...
# Configuration constants - uppercase with underscores for global variables
LOOKBACK_DAYS = 1  # Number of days to look back for reports (set to 1 for most recent day only)
INITIAL_SYNC_START_DATE = "2025-07-07"  # Start date for initial sync
REPORT_TYPES = ['blacklist', 'calls', 'audit', 'cases', 'account_risk', 'enrollment']  # All available report types
BASE_URL = "https://api.pindrop.com"  # Base URL for Pindrop API
RATE_LIMIT_DELAY = 1  # Rate limit delay in seconds between requests
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    log.info(f"Successfully obtained access token, expires at {self.token_expires_at}")
                    return self.access_token
                else:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        log.severe(f"OAuth2 token request failed after {max_retries} attempts: {error_msg}")
                        raise RuntimeError(f"Failed to obtain access token after {max_retries} attempts. Final error: {error_msg}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                log.warning(f"No data found for URL: {url}")
                return {"data": []} if not expect_csv else ""
            elif response.status_code == 401:
                # Token might be expired, let the caller handle token refresh
                raise RuntimeError(f"Authentication failed (401). Token may be expired.")
            else:
                log.warning(f"API request failed with status {response.status_code}: {response.text}")
                if attempt < max_retries - 1:
...
def parse_csv_data(csv_data: str, report_type: str, report_date: str) -> List[dict]:
    """
    ...
    try:
        ...
        log.info(f"Successfully parsed {len(records)} records from CSV for {report_type} on {report_date}")
        return records
...
def fetch_report_data(token_manager: OAuth2TokenManager, report_type: str, report_date: str) -> List[dict]:
    """
    ...
    try:
        ...
        if "Authentication failed" in str(e):
            ...
            try:
                ...
                log.info(f"Successfully fetched {len(records)} records for {report_type} on {report_date} after token refresh")
                return records
                
            except Exception as retry_error:
                log.severe(f"Failed to fetch {report_type} report for {report_date} even after token refresh: {str(retry_error)}")
                return []
        else:
...
def generate_reports_to_process(state: dict, is_initial_sync: bool) -> List[dict]:
    """
    ...
    if is_initial_sync:
        ...
        try:
            start_date = datetime.strptime(INITIAL_SYNC_START_DATE, '%Y-%m-%d').date()
            log.info(f"Initial sync: generating reports from {start_date} to {today}")
        except ValueError:
            log.severe(f"Invalid INITIAL_SYNC_START_DATE format: {INITIAL_SYNC_START_DATE}. Expected YYYY-MM-DD")
            return []
    else:
        # Incremental sync: lookback configured days
        start_date = today - timedelta(days=LOOKBACK_DAYS)
        log.info(f"Incremental sync: generating reports from {start_date} to {today} ({LOOKBACK_DAYS} days)")
        ...
        for report_type in REPORT_TYPES:
            ...
            if report_type in processed_reports:
                ...
                if last_processed_date and date_str <= last_processed_date:
                    log.info(f"Skipping {report_type} report for {date_str} - already processed up to {last_processed_date}")
                    continue
...
def schema(configuration: dict):
    ...
    for report_type in REPORT_TYPES:
        schemas.append({
            ...
            "primary_key": ["report_date", "id"] if report_type != 'blacklist' else ["report_date", "phone_number"]  # Primary key column(s) for the table, optional.
...
def update(configuration: dict, state: dict):
    ...
    token_manager = OAuth2TokenManager(client_id, client_secret)
    ...
    try:
        ...
        for report in reports_to_process:
            ...
            try:
                ...
                log.info(f"Completed processing {report_type} report for {report_date} - {len(records)} records")
