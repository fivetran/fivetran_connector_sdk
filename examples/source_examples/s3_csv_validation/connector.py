...
def update(configuration: dict, state: dict):
    ...
    response = s3_client.get_object(Bucket=configuration["bucket_name"], Key=configuration["file_key"])
...
def upsert_csv_row(row):
    int_value, int_validation_result = validate_int_value(row, row["int_column"])
    if not int_validation_result:
        yield None

    long_value, long_validation_result = validate_int_value(row, row["long_column"])
    if not long_validation_result:
        yield None

    bool_value, bool_validation_result = validate_bool_value(row, row["bool_column"])
    if not bool_validation_result:
        yield None

    string_value, string_validation_result = validate_string_value(row, row["string_column"])
    if not string_validation_result:
        yield None

    json_value, json_validation_result = validate_json_value(row, row["json_column"])
    if not json_validation_result:
        yield None

    native_date_value, native_date_validation_result = validate_native_date_value(row, row["native_date_column"])
    if not native_date_validation_result:
        yield None

    native_date_time_value, native_date_time_validation_result = validate_native_date_time_value(row, row[
        "native_date_time_column"])
    if not native_date_time_validation_result:
        yield None
    ...
    data = {
        ...
        "native_date_value": native_date_value.strftime("%Y-%m-%d"),
        "native_date_time_value": native_date_time_value.strftime("%Y-%m-%dT%H:%M:%S.%f")
    }
...
def validate_native_date_value(row, value: str):
    try:
        return datetime.strptime(value, "%Y-%m-%d").date(), True
    except ValueError:
        print_error_message(row, f"Invalid native date value: '{value}'")
        return None, False
...
def validate_native_date_time_value(row, value: str):
    try:
        ...
        print_error_message(row, f"Invalid native date time value: '{value}'")
        return None, False
