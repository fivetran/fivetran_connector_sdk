...
def update(configuration: dict, state: dict):
    ...
    try:
        ...
        for record in business_data:
            op.upsert(table="business_units", data=record)
        ...
        for record in reviews_data:
            op.upsert(table="reviews", data=record)
        ...
        if enable_consumer_reviews:
            ...
            for record in consumers_data:
                op.upsert(table="consumer_reviews", data=record)
        else:
        ...
        if enable_invitation_links:
            ...
            for record in invitations_data:
                op.upsert(table="invitation_links", data=record)
        else:
