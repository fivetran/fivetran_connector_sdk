...
LEFT JOIN review r ON bu.business_unit_id = r.business_unit_id
...
SELECT 
    ...
    COUNT(*) as review_count,
    (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM review WHERE timestamp::timestamp >= CURRENT_DATE - INTERVAL '30 days')) as percentage,
    AVG(helpful_count) as avg_helpful_count,
...
SELECT 
    ...
    c.localized_name,
    c.level,
    c.created_at::timestamp as category_created,
    c.updated_at::timestamp as last_updated
FROM category c
ORDER BY c.name;
...
FROM consumer_review c
LEFT JOIN review r ON c.consumer_id = r.consumer_id AND c.business_unit_id = r.business_unit_id
...
SELECT 
    ...
    i.status,
    COUNT(*) as count,
    (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM invitation_link WHERE timestamp::timestamp >= CURRENT_DATE - INTERVAL '30 days')) as percentage,
    AVG(EXTRACT(EPOCH FROM (i.responded_at::timestamp - i.sent_at::timestamp)) / 3600) as avg_response_hours
FROM invitation_link i
...
WITH business_metrics AS (
    ...
    LEFT JOIN invitation_link i ON bu.business_unit_id = i.business_unit_id 
