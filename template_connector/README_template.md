...
> Note : If configuration is required to run the connector, ensure you have added a `configuration.json` file with the necessary parameters before running `fivetran debug` for the first time. Refer to the [Configuration file](#configuration-file) section below for details on the required configuration parameters.
