...
*Do not use Title Case in plain text that is used as unformatted subheading or item title, only capitalize the firs word of subheading/item title (e.g., \- Rate limit errors – The connector automatically retries with exponential backoff, \- Data Volume – Large accounts may require longer sync times, Incremental sync:).*
